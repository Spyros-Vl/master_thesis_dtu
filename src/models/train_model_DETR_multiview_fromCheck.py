...
import sys
sys.path.append('../')
from src.data.my_rpg_dataset import MultiViewCocoDetection
...
def main(best_loss):
    ...
    train_dataset = MultiViewCocoDetection(path_folder="data", processor=processor,status="train")
    ...
    validation_dataset = MultiViewCocoDetection(path_folder="data", processor=processor,status="validation")
    ...
    best_model = torch.load(f'Last_MultiView_DETR_Model.pt')
    ...
    for epoch in range(NumOfEpochs):
        ...
        if validation_loss > best_loss:
            ...
            torch.save(checkpoint,f'Best_val_MultiView_DETR_Model.pt')
    ...
    torch.save(checkpoint,f'Last_MultiView_DETR_Model.pt')
    print("Model state saved on epoch: ", (epoch+1))
